     ...
     $sql = "SELECT * FROM agence WHERE id_admin_ag = '".$idadmin."'";
       $result =Model::$pdo->query($sql) or die (Model::$pdo->error());
          while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            <a class="btn btn-danger btn-sm" title="Refuser ce agence" href="index.php?controller=admin&action=refuser_agence&id='.$row['id_agence'].'" onclick="return confirm(\'Vous avez refuser\')">Refuser</a>';
                          }
                            echo '</td>';
                            echo '</tr> ';
                }
